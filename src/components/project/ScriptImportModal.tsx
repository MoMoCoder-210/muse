 ...
 */
export function ScriptImportModal({ project, onClose, onImported }: ScriptImportModalProps) {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      if (!settings.text?.apiKey?.trim()) {
        toast("文本模型未配置，请先到设置页填入后再启动拆分。", "warning");
        return;
      }
  ...
  }, [tab, pasteText, filePath, project.id, onImported, onClose, toast]);

  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true" onClick={onClose}>
      <div className="modal-panel script-import-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="script-import-modal-desc">将剧本文本导入后，系统会自动按集数拆分为片段</p>
