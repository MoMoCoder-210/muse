...
import type {
  AssetNodeData, CanvasCenterNodeData, ClipNodeData, EpisodeEntryNodeData, ImageNodeData, MaterialCategoryNodeData,
...
export const CanvasCenterNode = memo(function CanvasCenterNode({ data }: NodeProps<CanvasCenterNodeData>) {
  ...
  return <section className={`cn-center cn-center--${data.centerKind}${expanded ? "" : " cn-center--collapsed"}`} aria-label={data.title}>
    <header className="cn-center__header"><span className="cn-center__eyebrow">PRODUCTION CANVAS</span><strong>{data.title}</strong><span>{data.itemCount} 项</span><button className="cn-center__toggle nodrag nopan" type="button" aria-label={expanded ? `收起 ${data.title}` : `展开 ${data.title}`} aria-expanded={expanded} onClick={toggle}><ChevronIcon /></button></header>
...
export const ClipNode = memo(function ClipNode({ data }: NodeProps<ClipNodeData>) {
  ...
  return <CardShell canonicalId={data.canonicalId} expansionId={data.expansionId} hasChildren color={tone(clipStatusColor(data.status ?? "pending"))} className="cn-card--clip" icon={<ClipIcon />} title={data.title || "未命名分集"} subtitle={`${data.assetCount} 素材 · ${data.storyboardCount} 镜头 · ${duration}`} badge={data.status} />;
});
export const EpisodeEntryNode = memo(function EpisodeEntryNode({ data }: NodeProps<EpisodeEntryNodeData>) {
  const labels = { materials: "素材中心", shots: "镜头中心", release: "成片中心" };
  return <CardShell canonicalId={data.canonicalId} expansionId={data.expansionId} hasChildren color={data.centerKind === "materials" ? "#b69a70" : data.centerKind === "shots" ? "#78a8cf" : "#8da99b"} className={`cn-card--episode-entry cn-card--episode-entry-${data.centerKind}`} icon={<EntryIcon />} title={`${labels[data.centerKind]} · 入口`} subtitle={data.detailOpen ? `${data.itemCount} 项 · 展开` : `${data.itemCount} 项 · 摘要`} badge={data.detailOpen ? undefined : "已收起"} />;
});
...
export const AssetNode = memo(function AssetNode({ data }: NodeProps<AssetNodeData>) {
  const { expanded } = useExpansion(data.canonicalId);
  const imageInfo = data.selectedImagePath ? "主图已选" : data.imageCount ? `${data.imageCount} 张图` : "待生成";
  const visibleImages = data.previewImages.slice(0, 4);
  const visibleTasks = data.taskPreviews.slice(0, 2);
  return <CardShell canonicalId={data.canonicalId} hasChildren={data.hasChildren} color={tone(assetTypeColor(data.type))} className={`cn-card--asset${expanded ? " cn-card--asset-expanded" : ""}`} icon={<AssetIcon type={data.type} />} title={data.name} subtitle={`${assetTypeLabel(data.type)} · ${imageInfo}`} badge={data.status} thumbnail={data.selectedImagePath ? <img src={mediaSrc(data.selectedImagePath)} alt="" loading="lazy" draggable={false} /> : <span className="cn-asset-placeholder"><AssetIcon type={data.type} /></span>}>
    {expanded && <div className="cn-asset-details nodrag nopan">
      {visibleImages.length > 0 && <div className="cn-asset-details__images" aria-label={`${data.name} 的图片`}>
        {visibleImages.map((image) => <img key={image.id} className={image.isSelected ? "cn-asset-details__image--selected" : ""} src={mediaSrc(image.imagePath)} alt="" loading="lazy" draggable={false} title={image.isSelected ? "当前主图" : image.size || "素材图片"} />)}
        {data.imageCount > visibleImages.length && <span className="cn-asset-details__more">+{data.imageCount - visibleImages.length}</span>}
      </div>}
      {visibleTasks.length > 0 && <div className="cn-asset-details__tasks">{visibleTasks.map((task) => <span key={task.id} className={`cn-asset-task cn-asset-task--${task.status}`} title={task.error || task.taskType}>{task.taskType} · {taskLabel(task.status)}</span>)}</div>}
      {data.description && <p className="cn-asset-details__description">{data.description}</p>}
    </div>}
  </CardShell>;
});
...
export const VideoNode = memo(function VideoNode({ data }: NodeProps<VideoNodeData>) { const ready = data.filePath && (!data.isUpscaleOutput || data.isOutputReady); return <CardShell canonicalId={data.canonicalId} hasChildren={false} color="#7895ad" className="cn-card--task" icon={<PlayIcon />} title={data.fileName || "镜头视频"} subtitle={data.duration ? `${Math.round(data.duration)} 秒` : "视频产物"} thumbnail={ready ? <video src={mediaSrc(data.filePath)} muted preload="metadata" playsInline /> : undefined} />; });
...
export const agentNodeTypes = { CanvasCenterNode, ClipNode, EpisodeEntryNode, MaterialCategoryNode, MaterialSpineNode, AssetNode, ImageNode, StoryboardNode, ShotTrackNode, ShotAnchorNode, VideoNode, TaskNode, ReleaseSummaryNode, ReleaseTaskNode, ReleaseOutputNode };
...
function EntryIcon() { return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M4 12h14M14 7l5 5-5 5"/><circle cx="5" cy="12" r="2"/></svg>; }
