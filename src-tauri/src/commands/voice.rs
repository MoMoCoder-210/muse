...
fn load_voice_config(app: &AppHandle) -> Result<VoiceConfig, String> {
    ...
    let api_key = active.get("apiKey").and_then(|v| v.as_str()).unwrap_or("").trim().to_string();
    let resource_id = active.get("resourceId").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let base_url = active.get("baseUrl").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let sample_rate = active.get("sampleRate").and_then(|v| v.as_u64()).unwrap_or(24000) as u32;
    ...
    Ok(VoiceConfig { api_key, resource_id, base_url, sample_rate, timeout_ms })
}
...
pub fn seed_public_voices(conn: &rusqlite::Connection) -> Result<(), String> {
    ...
    let voices: serde_json::Value = serde_json::from_str(json_str)
        .map_err(|e| format!("解析公共音色清单失败：{}", e))?;
    let arr = voices.as_array().ok_or("公共音色清单格式错误（应为数组）")?;
    ...
    for v in arr {
        ...
        let tags = v.get("tags").map(|x| x.to_string()).unwrap_or_else(|| "[]".to_string());
...
pub fn preview_public_voice(
    ...
    if let Some(p) = resolve_bundled_voice(&app, &voice_id) {
        ...
        crate::project_log::append_log(&log_path, "语音", "INFO",
            &format!("试听命中打包离线音频：voiceId={} path={}", voice_id, p_str));
        return Ok(PreviewVoiceResult { sample_path: p_str, cached: true });
    }
    ...
    if sample_path.exists() {
        crate::project_log::append_log(&log_path, "语音", "INFO",
            &format!("试听命中本地缓存：voiceId={} path={}", voice_id, sample_path_str));
        return Ok(PreviewVoiceResult { sample_path: sample_path_str, cached: true });
    }
    ...
    crate::project_log::append_log(&log_path, "语音", "INFO",
        &format!("试听合成请求(V3)：voiceId={} url={} resourceId={}", voice_id, url, resource_id));

    let audio_bytes = synthesize_via_v3(&url, &cfg.api_key, &resource_id, &voice_id, cfg.sample_rate, cfg.timeout_ms, &log_path)?;
    ...
    let save_path_str = if std::fs::write(&save_path, &audio_bytes).is_ok() {
        ...
        let fallback = app_data_dir.join(VOICES_DIR).join(format!("{}.mp3", sanitize_voice_filename(&voice_id)));
        std::fs::create_dir_all(fallback.parent().unwrap_or(&fallback)).map_err(|e| format!("创建音频缓存目录失败：{}", e))?;
    ...
    crate::project_log::append_log(&log_path, "语音", "INFO",
        &format!("试听合成成功并缓存：voiceId={} path={} bytes={}", voice_id, save_path_str, audio_bytes.len()));

    Ok(PreviewVoiceResult { sample_path: save_path_str, cached: false })
}
...
fn synthesize_via_v3(
        ...
        .send_json(body)
    {
        ...
        Err(ureq::Error::Status(status, r)) => {
            let detail = r.into_string().unwrap_or_else(|e| format!("<读取响应体失败: {}>", e));
            let msg = classify_voice_error(status, &detail);
            crate::project_log::append_log(&log_path, "语音", "ERROR",
                &format!("试听合成失败（V3 HTTP {}）：voiceId={} detail={}", status, voice_id, detail));
            return Err(msg);
        }
        Err(e) => {
    ...
    if status < 200 || status >= 300 {
        let detail = resp.into_string().unwrap_or_else(|e| format!("<读取响应体失败: {}>", e));
        let msg = classify_voice_error(status, &detail);
        crate::project_log::append_log(&log_path, "语音", "ERROR",
            &format!("试听合成 HTTP 错误：voiceId={} status={} detail={}", voice_id, status, detail));
        return Err(msg);
    }

    let mut raw = Vec::new();
    resp.into_reader().read_to_end(&mut raw).map_err(|e| format!("读取音频流失败：{}", e))?;
    ...
    let audio_bytes: Vec<u8> = if raw.first().copied() == Some(b'{') {
        ...
        for item in serde_json::Deserializer::from_slice(&raw).into_iter::<serde_json::Value>() {
            let v = match item {
                Ok(v) => v,
                Err(e) => { err_log = Some((-1, format!("解析语音响应片段失败：{}", e))); break; }
            };
            ...
            if let Some(d) = v.get("data").and_then(|d| d.as_str()) { data_b64.push_str(d); }
        }
        if let Some((code, msg)) = err_log {
            crate::project_log::append_log(&log_path, "语音", "ERROR",
                &format!("试听合成失败（V3 code={}）：voiceId={} message={}", code, voice_id, msg));
            return Err(format!("语音合成失败：{}", msg));
        }
        if data_b64.is_empty() { return Err("语音响应未包含任何音频数据（data 为空）".into()); }
        base64::engine::general_purpose::STANDARD.decode(data_b64).map_err(|e| format!("解码语音音频失败：{}", e))?
    } else {
        raw
    };
    ...
    if !is_mp3 {
        crate::project_log::append_log(&log_path, "语音", "ERROR",
            &format!("试听合成返回非 mp3 数据：voiceId={} head={:02X?}", voice_id, &audio_bytes[..audio_bytes.len().min(4)]));
        return Err("语音合成返回的数据不是 mp3 音频".into());
    }
    if audio_bytes.is_empty() {
        crate::project_log::append_log(&log_path, "语音", "ERROR", &format!("试听合成返回空音频：voiceId={}", voice_id));
...
fn classify_voice_error(status: u16, detail: &str) -> String {
    ...
    if status == 401 {
        return format!(
            "语音合成鉴权失败（HTTP 401）detail={}",
            detail
        );
    }
    if status == 403 {
        return format!(
            "语音合成被拒绝（HTTP 403）detail={}",
            detail
        );
    }
    if status == 404 {
        return format!(
            "语音合成失败（HTTP 404）detail={}",
            detail
        );
    }
...
fn upsert_sample_path(conn: &rusqlite::Connection, voice_id: &str, path: &str) -> rusqlite::Result<()> {
    conn.execute(
        "INSERT INTO public_voices (voice_id, name, gender, age, language, tags, sample_audio_path) \
         VALUES (?1, ?1, '', '', '', '[]', ?2) \
