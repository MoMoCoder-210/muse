...
对生成的镜头视频进行 AI 高清放大，提升最终成片画质：

- **本地推理**：基于 [Real-ESRGAN](https://github.com/xinntao/Real-ESRGAN) 的 ncnn-vulkan 引擎，无需联网，数据不出本机
- **多模型**：
    - `动漫视频`（realesr-animevideov3）：轻量快速，适合动漫/国漫，支持 2x/3x/4x、不限分辨率
    - `动漫高清`（x4plus-anime）：动漫专用高清模型，固定 4x
    - `真人写实`（x4plus）：通用写真模型，固定 4x、限 1080p 以内
- **倍率选择**：2x / 3x / 4x，可在发起时自由选择
- **任务队列**：支持同时发起多个视频超分，任务串行排队执行，实时显示进度
- **断点续跑**：任务中断后重启应用自动从断点继续，仅补算未完成的帧，不重复计算
- **一键管理**：超分产物作为独立批次展示，支持预览、删除与绑定镜头
